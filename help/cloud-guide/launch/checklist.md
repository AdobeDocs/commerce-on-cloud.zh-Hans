...
Adobe为每个环境提供让我们加密SSL/TLS证书。 Fastly需要此证书才能通过HTTPS提供安全流量。
...
- 为您的生产环境配置的SSL/TLS证书。
  ...
  如果您在Fastly设置过程中添加了生产域的ACME质询记录，则当您更新DNS配置以将流量路由到Fastly服务时，Adobe会自动将SSL/TLS证书上传到您的生产环境。 如果您没有预配置证书，或者您更新了域，则Adobe必须完成域验证并配置证书，这最多可能需要12小时。
...
1. 如果需要，添加A和AAAA记录以将Apex域(`<domain-name>.com`)映射到以下Fastly IP地址：
