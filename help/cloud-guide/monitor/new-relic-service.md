...
>
>Cloud上的Adobe Commerce客户无法将外部服务器中的数据直接集成或发送到其配置的New Relic帐户中。 New Relic服务仅限用于监控Commerce Cloud应用程序环境。 只有应用程序本身调用的第三方服务（例如，运行时调用的外部API或服务）才能在New Relic中跟踪和监控。
